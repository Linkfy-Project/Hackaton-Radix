...
1) DASHBOARD:
   - Mantém TUDO que já existia (MMGD por visão + perfis + risco).
   - USA a MMGD "real" (BDGD/GeoJSON) para COMPARAR com visão computacional:
     * KPIs de BDGD, Δ (visão - BDGD), cobertura (quantas SEs têm BDGD)
     * Scatter BDGD vs Visão (kW) + correlação
     * Ranking de discrepâncias (Δ kW e Δ %)
     * Colunas opcionais na tabela: BDGD_MMGD_KW / MMGD_DELTA_KW / MMGD_RATIO_CV_VS_BDGD / MMGD_DELTA_PCT
2) MAPA:
   - Continua usando RISCO_MMGD_NIVEL (visão/SE) como overlay.
   - Popup completo continua mostrando BDGD (se existir) + visão.
3) SOLAR (Visão Computacional):
   - Mantém parâmetros configuráveis na UI (MAX_WORKERS_DOWNLOAD, MAX_TILES, ZOOM, conf, COD_IDs).
Obs:
- "BDGD_MMGD_KW" é detectado automaticamente em dados_finais_rj.geojson por nomes comuns de coluna.
- Se sua coluna de MMGD real tiver nome diferente, renomeie no GeoJSON ou acrescente ao detector em detect_mmgd_real_column().
"""
...
def merge_enrichment(
    ...
    # BDGD MMGD (REAL) — detecta coluna no GeoJSON
...
C) Comparação Visão vs BDGD (se existir no GeoJSON):
   - BDGD_MMGD_KW = coluna detectada automaticamente
   - Δ kW = CV_MMGD_EST_KW - BDGD_MMGD_KW
   - Razão = CV_MMGD_EST_KW / BDGD_MMGD_KW
"""
    os.makedirs(PASTA_PROC, exist_ok=True)
...
- **Comparação BDGD (real) vs Visão**: quando o GeoJSON possuir potência MMGD real (coluna detectada automaticamente).
"""
    )
...
# TAB 3 — DASHBOARD (COM BDGD MMGD para comparação)
with tabs[2]:
    st.subheader("Dashboard (Visão Computacional + BDGD)")
    ...
    if df_resumo is None or df_resumo.empty:
        ...
        # --- comparação BDGD vs Visão ---
        if "BDGD_MMGD_KW" in dff.columns:
            dff["BDGD_MMGD_KW"] = pd.to_numeric(dff["BDGD_MMGD_KW"], errors="coerce").fillna(0.0)
        else:
            dff["BDGD_MMGD_KW"] = 0.0

        dff["MMGD_DELTA_KW"] = pd.to_numeric(dff.get("CV_MMGD_EST_KW", 0), errors="coerce").fillna(0.0) - dff["BDGD_MMGD_KW"]
        den = dff["BDGD_MMGD_KW"].replace(0, np.nan)

        dff["MMGD_RATIO_CV_VS_BDGD"] = np.nan_to_num(
            (pd.to_numeric(dff.get("CV_MMGD_EST_KW", 0), errors="coerce").fillna(0.0) / den),
            nan=0.0, posinf=0.0, neginf=0.0
        )
        dff["MMGD_DELTA_PCT"] = np.nan_to_num(
            (dff["MMGD_DELTA_KW"] / den) * 100.0,
            nan=0.0, posinf=0.0, neginf=0.0
        )
        ...
        k5.metric("MMGD BDGD total (kW)", f"{_num_sum(dff, 'BDGD_MMGD_KW'):,.0f}".replace(",", "."))
        k6.metric("Δ total (Visão - BDGD) (kW)", f"{_num_sum(dff, 'MMGD_DELTA_KW'):,.0f}".replace(",", "."))
        k7.metric("Cobertura BDGD (SEs com >0 kW)", f"{int((dff['BDGD_MMGD_KW'] > 0).sum()):,}".replace(",", "."))

        tmp_corr = dff[(dff["BDGD_MMGD_KW"] > 0) & (pd.to_numeric(dff.get("CV_MMGD_EST_KW", 0), errors="coerce") > 0)]
        corr = tmp_corr["BDGD_MMGD_KW"].corr(pd.to_numeric(tmp_corr["CV_MMGD_EST_KW"], errors="coerce")) if len(tmp_corr) >= 3 else 0.0
        k8.metric("Correlação BDGD vs Visão", f"{(corr if pd.notna(corr) else 0.0):.2f}")

        st.divider()
        st.markdown("### 📊 Gráficos (Visão Computacional + BDGD + Perfis)")
        ...
        if px is None:
            ...
            # (E) Comparação BDGD vs Visão + (F) Ranking de Δ
            g5, g6 = st.columns(2)
            with g5:
                tmp = dff.copy()
                tmp["BDGD_MMGD_KW"] = pd.to_numeric(tmp["BDGD_MMGD_KW"], errors="coerce").fillna(0.0)
                tmp["CV_MMGD_EST_KW"] = pd.to_numeric(tmp.get("CV_MMGD_EST_KW", 0), errors="coerce").fillna(0.0)
                tmp = tmp[(tmp["BDGD_MMGD_KW"] > 0) | (tmp["CV_MMGD_EST_KW"] > 0)]
                if tmp.empty:
                    st.info("Sem pontos para comparar BDGD vs Visão.")
                else:
                    st.plotly_chart(
                        px.scatter(
                            tmp,
                            x="BDGD_MMGD_KW",
                            y="CV_MMGD_EST_KW",
                            title="Comparação MMGD — BDGD (real) vs Visão (kW)",
                            hover_data=[c for c in ["COD_ID","NOM","DISTRIBUIDORA","MMGD_DELTA_KW","MMGD_DELTA_PCT"] if c in tmp.columns],
                        ),
                        use_container_width=True,
                    )

            with g6:
                top = dff.copy().sort_values("MMGD_DELTA_KW", ascending=False).head(15)
                if top.empty:
                    st.info("Sem discrepâncias para ranking.")
                else:
                    st.plotly_chart(
                        px.bar(
                            top,
                            x="NOM" if "NOM" in top.columns else "COD_ID",
                            y="MMGD_DELTA_KW",
                            title="Top 15 — Δ (Visão - BDGD) em kW",
                            hover_data=[c for c in ["COD_ID","DISTRIBUIDORA","BDGD_MMGD_KW","CV_MMGD_EST_KW","MMGD_DELTA_PCT"] if c in top.columns],
                        ),
                        use_container_width=True,
                    )
        ...
        # ✅ Tabela principal (mantém tudo + adiciona comparação)
        st.markdown("#### 📌 Tabela principal (perfil + MMGD visão + BDGD + risco + comparação)")
        display_cols = {
            ...
            "BDGD_MMGD_KW": "MMGD BDGD (kW)",
            "MMGD_DELTA_KW": "Δ (Visão - BDGD) kW",
            "MMGD_RATIO_CV_VS_BDGD": "Razão (Visão/BDGD)",
            "MMGD_DELTA_PCT": "Δ % (Visão vs BDGD)",
        ...
        if "RISCO_MMGD_RATIO" in dff.columns:
            ...
            show = ["COD_ID","NOM","DISTRIBUIDORA","SUB_CAP_KW","CV_MMGD_EST_KW","RISCO_MMGD_RATIO","RISCO_MMGD_NIVEL","conf_media","area_total_m2","qtd_paineis","BDGD_MMGD_KW","MMGD_DELTA_KW"]
        ...
        st.markdown("#### 🧾 Ranking — maior discrepância (Visão vs BDGD)")
        top_delta = dff.copy().sort_values("MMGD_DELTA_KW", ascending=False).head(20)
        show2 = ["COD_ID","NOM","DISTRIBUIDORA","BDGD_MMGD_KW","CV_MMGD_EST_KW","MMGD_DELTA_KW","MMGD_DELTA_PCT","conf_media","area_total_m2","qtd_paineis"]
        show2 = [c for c in show2 if c in top_delta.columns]
        st.dataframe(top_delta[show2], use_container_width=True, height=360)

        st.markdown("#### 🧾 Ranking — maior subestimação (Visão abaixo do BDGD)")
        top_neg = dff.copy().sort_values("MMGD_DELTA_KW", ascending=True).head(20)
        st.dataframe(top_neg[show2], use_container_width=True, height=360)
