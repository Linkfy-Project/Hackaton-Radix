...
def pre_process():
    ...
    # 3. Carregar CNEFE em chunks com barra de progresso
    print("DEBUG: Lendo TODOS os pontos do CNEFE e associando às áreas...")
    chunk_size = 200000
    
    # Contar linhas para a barra de progresso
    total_rows = 8962201 # Valor obtido via wc -l
    
    all_stats = []
    sample_points = []
    
    # Usamos tqdm para a barra de progresso
    with tqdm(total=total_rows, desc="Processando CNEFE") as pbar:
        for chunk in pd.read_csv(CAMINHO_CNEFE, sep=';', usecols=['LATITUDE', 'LONGITUDE', 'COD_ESPECIE'], chunksize=chunk_size):
            ...
            # Converter chunk para GeoDataFrame
